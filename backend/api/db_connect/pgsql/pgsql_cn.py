...
pg = APIRouter()
# 连接用户的pgsql数据库,并获取数据库的表名，例如
# postgresql+psycopg2://postgres:123456@127.0.0.1:5432/postgres
# postgresql+psycopg2://user:123456@127.0.0.1:5432/data
...
@pg.get("/{dataAssetId}/tablesInfo",response_model=FieldsInfo)
async def get_tables_info(dataAssetId: int):
    engine = None
    try:
        data_asset = await DA.get(id=dataAssetId).prefetch_related("datasource")
        ...
        # 初始化模型
        fields_info = FieldsInfo(dataAssetId=dataAssetId,tableName=data_asset.name)
        # 添加字段信息
        for col in columns:
